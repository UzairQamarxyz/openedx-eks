...
data "aws_iam_policy_document" "default_kms_key_policy" {
  count = var.create_default_key ? 1 : 0
  statement {
    actions = [
      "kms:Create*",
      "kms:Describe*",
...
data "aws_iam_policy_document" "elasticache_kms_key_policy" {
  count = var.create_elasticache_key ? 1 : 0
  statement {
    actions = [
      "kms:Create*",
      "kms:Describe*",
...
data "aws_iam_policy_document" "aoss_kms_key_policy" {
  ...
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "kms:ViaService"
      values   = ["aoss.${data.aws_region.current.region}.amazonaws.com"]
    }
  }
}

data "aws_iam_policy_document" "firewall_kms_key_policy" {
  count = var.create_firewall_key ? 1 : 0
  statement {
    actions = [
      "kms:Create*",
      "kms:Describe*",
  ...
  statement {
    ...
    resources = ["*"]
    sid       = "Allow access through firewall for all principals in the account that are authorized to use firewall"
    principals {
      type        = "AWS"
      identifiers = ["*"]
    }
    ...
    condition {
      test     = "StringEquals"
      variable = "kms:ViaService"
      values   = ["network-firewall.${data.aws_region.current.region}.amazonaws.com"]
    }
  }
}
